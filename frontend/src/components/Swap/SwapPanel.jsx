...
const TOKENS = [
    ...
    { symbol: 'ARENA', color: '#9ffe6a', textColor: 'text-dark' },
    { symbol: 'USDT', color: '#26a17b', textColor: 'text-white' },
];

export const SwapPanel = ({ account, walletBalance, bnbPrice }) => {
    ...
    const displayAddr = account ? `${account.slice(0, 6)}...${account.slice(-4)}` : '0x...connect';
    const available = walletBalance !== null && walletBalance !== undefined ? walletBalance.toFixed(4) : '0.0000';
    const rate = 600;
    const numAmount = parseFloat(amount) || 0;
    const receiveAmount = (numAmount * rate).toFixed(2);
    const dollarValue = bnbPrice ? (numAmount * bnbPrice).toFixed(2) : (numAmount * rate).toFixed(2);
    ...
    return (
        ...
        >
            <div className="flex justify-between items-center mb-8">
                <h2 className="text-2xl font-medium text-white">Acquire ARENA</h2>
            ...
            <div className="space-y-2 relative">
                ...
                <div className="bg-[#12231a] rounded-3xl p-5 border border-white/5 hover:border-white/10 transition-colors">
                    <div className="flex justify-between text-xs text-gray-400 mb-3 font-medium tracking-wide">
                        ...
                        <div className="flex items-center gap-2 bg-dark/50 px-3 py-1.5 rounded-full border border-white/5 cursor-pointer hover:bg-white/10 transition-colors">
                            ...
                            <span className="text-white">{displayAddr}</span>
                        </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button type="button" onClick={() => setAmount(available)} className="text-gray-400 hover:text-primary transition-colors cursor-pointer">MAX</button>
                        </div>
                    </div>
                ...
                <div className="bg-[#12231a] rounded-3xl p-5 border border-white/5 hover:border-white/10 transition-colors">
                    <div className="flex justify-between text-xs text-gray-400 mb-3 font-medium tracking-wide">
                        <span>TO:</span>
                        <div className="flex items-center gap-2 bg-dark/50 px-3 py-1.5 rounded-full border border-white/5 cursor-pointer hover:bg-white/10 transition-colors">
                            <div className="w-4 h-4 rounded-full bg-[#9ffe6a] flex items-center justify-center text-[8px] text-dark font-bold">A</div>
                            <span className="text-white">Arena Pool</span>
                        </div>
                    </div>
            ...
            <div className="mt-auto mb-6">
                ...
                <button
                    ...
                    disabled={!numAmount}
                    className={`w-full text-lg font-bold py-4 rounded-2xl flex items-center justify-between px-6 transition-all hover:scale-[1.02] shadow-[0_0_20px_rgba(159,254,106,0.2)] cursor-pointer ${confirmed ? 'bg-green-500 text-white' : !numAmount ? 'bg-gray-700 text-gray-400 cursor-not-allowed hover:scale-100' : 'bg-primary hover:bg-primaryDark text-dark'
                        }`}
                >
                    <span>{confirmed ? 'Arena Token Minted!' : !numAmount ? 'Enter Amount' : 'Confirm Stake'}</span>
